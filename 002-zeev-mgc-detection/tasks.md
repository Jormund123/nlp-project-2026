...
- [ ] T009 [Day1] Implement `train_tfidf_baseline()` in `src/baseline_tfidf_b.py`
...
- [ ] T010 [Day1] Train TF-IDF + LogReg on Task B subset (use raw `code` column)
- [ ] T011 [Day1] Evaluate and print Macro F1 (expected ~0.40-0.60)
- [ ] T012 [Day1] Save TF-IDF vectorizer and classifier to outputs

**Checkpoint**: ✅ TF-IDF Macro F1 recorded
...
- [ ] T013 [Day1] Set up CodeBERT tokenizer (`microsoft/codebert-base`) and config max_length=512
- [ ] T014 [Day1] Implement `CodeDataset` class in `src/train_task_b.py` (handles `code` vs `code_preprocessed`)
- [ ] T015 [Day1] Verify batch tokenization shape `(32, 512)` on a batch
- [ ] T016 [Day1] Implement `CodeBERTClassifier` model class for multiclass (768 → 11)
- [ ] T017 [Day1] Implement training loop with AdamW optimizer
	- Config: `lr=2e-5, weight_decay=0.01, warmup_ratio=0.1`
- [ ] T018 [Day1] Test `train_task_b.py` runs 1 epoch without crash (smoke test) and save small checkpoint `model_task_b_epoch1.pt`
- [ ] T019 [Day1] (Optional) Run a short 1k-sample epoch to sanity-check loss decrease
...
- [ ] T020 [Day1] Implement `evaluate_model()` in `src/evaluate.py` (return Macro F1, precision, recall, confusion_matrix, per-class metrics)
- [ ] T021 [Day1] Evaluate TF-IDF on validation set → record Macro F1, per-class metrics
- [ ] T022 [Day1] Evaluate CodeBERT on validation set → record Macro F1, per-class metrics
...
- [ ] T024 [Day1] Fill baseline comparison summary for poster

**Checkpoint**: ✅ Evaluation metrics saved and per-generator table produced
...
- [ ] T025 [Day2] Add `--preprocess` flag to `train_task_b.py` (default = raw)
- [ ] T026 [Day2] Train CodeBERT WITH preprocessing and save checkpoint `model_task_b_preprocessed.pt`
- [ ] T027 [Day2] Evaluate preprocessed model on validation set and compare Macro F1
- [ ] T028 [Day2] Fill ablation table and write 1-sentence insight for poster
...
- [ ] T029 [Day2] Compute per-generator Macro F1 and save CSV/PNG (11 rows)
- [ ] T030 [Day2] Compute and save confusion matrix (raw counts + row-normalized) as PNG and `.npy`
- [ ] T031 [Day2] Compute per-language Macro F1 breakdown and save table
- [ ] T032 [Day2] Identify top-3 confusion pairs and write `notes/top3_confusions.md`
...
- [ ] T033 [Day2] Implement `generate_confusion_matrix()` and save `confusion_matrix_task_b.png` and `.npy`
- [ ] T034 [Day2] Implement `plot_per_generator_f1()` and save PNG/table for poster
- [ ] T035 [Day2] Generate training loss curves and save `loss_curve_b.png`
- [ ] T036 [Day2] Generate baseline comparison figure and save `baseline_comparison_b.png`
