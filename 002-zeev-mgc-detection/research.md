...
# Research & Technology Decisions: MGC Detection (Zeev)

## Focus

This document states decisions relevant to Zeev's responsibilities: Task B multiclass detection, evaluation metrics, visualization standards, and training configuration for CodeBERT on Task B.

## Model Selection

- Use `microsoft/codebert-base` as encoder. Rationale: pretrained on code, lightweight and consistent with Team baseline.
- Input truncation: first 512 tokens by default. For Task B, generator artifacts (style) tend to appear early in files (docstrings, headers, imports). If time permits, evaluate sliding-window pooling as an ablation.

## Multiclass Considerations (Task B)

- Metric: Macro F1 (primary) — preserves equal weight for rare generators.
- Confusion analysis: produce 11×11 confusion matrix and normalized (row-wise) confusion to highlight which generators are confused.
- Class imbalance handling: consider `class_weight` in LogisticRegression for TF-IDF baseline; for CodeBERT, up/down-sampling or per-class loss weighting can be considered if needed.

## TF-IDF Configuration (Task B baseline)

- TfidfVectorizer config: max_features=10000, ngram_range=(1,3), analyzer='word', token_pattern=r"\b\w+\b", min_df=2.
- Use `LogisticRegression(max_iter=1000, class_weight='balanced', solver='lbfgs')` and report macro F1.

## CodeBERT Training Config (Task B)

- Batch size: 32 (adjust if GPU memory limited).
- Epochs: 3 for full runs; use 1-epoch smoke tests.
- Learning rate: 2e-5, weight_decay=0.01, warmup_ratio=0.1.
- Loss: CrossEntropyLoss for multiclass with option for class weights.

## Visualization Standards

- Use Matplotlib + Seaborn; consistent style (`seaborn-v0_8-whitegrid`) and colorblind palette.
- FIGURE_DPI = 300, FIGURE_SIZE=(8,6), FONT_SIZE=12.
- Color scheme: TF-IDF '#1f77b4', CodeBERT '#ff7f0e', raw '#2ca02c', preprocessed '#7f7f7f'.

## Outputs & Reproducibility

- Save figures and CSVs under `outputs/002-zeev-mgc-detection` with explicit filenames.
- Log command-line arguments and stdout to `outputs/002-zeev-mgc-detection/logs/run_YYYYMMDD_HHMMSS.log`.

## Small Risks & Mitigations

- Risk: Some generators have few samples making per-generator F1 noisy. Mitigation: report sample counts and avoid overinterpreting tiny classes; consider merging extremely small generators for analysis footnotes.
- Risk: GPU memory limits. Mitigation: run smoke tests on CPU/MPS or smaller batch size; use mixed precision when possible.
